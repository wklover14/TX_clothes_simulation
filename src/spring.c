...
/**
 * Add to the table springs all the possible springs from the index i,j in a n*m matrix and complete the face_index matrix as follow,
 * structural springs  index are in position O -> 3, shear springs in position 4, 5, and Flexion springs in position 6 -> 7
 */
void fillSprings(Spring* springs, unsigned int*** face_spring_indices, unsigned int* spring_index, int i, int j, int n, int m)
{
    Point current = {i, j};
    ...
    if (i + 2 < n) {
        ...
        if (j < m - 1) {
            face_spring_indices[i][j][6] = *spring_index; // Horizontal spring spanning two cells
        }
        (*spring_index)++;
    }
    ...
    if (j + 2 < m) {
        ...
        // Face (i, j) affected by this flexion spring
        if (i < n - 1) {
            face_spring_indices[i][j][7] = *spring_index; // Vertical spring spanning two cells
        }
        (*spring_index)++;
    }
}
